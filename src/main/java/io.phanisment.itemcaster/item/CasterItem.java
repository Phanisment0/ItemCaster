...
public class CasterItem {
	...
	public void applyData(MythicMobItemGenerateEvent e) {
		...
		if (Constants.hasResourcePack()) {
			if (model_data.hasModelData()) meta.setCustomModelData(model_data.getModelData());
			if (ServerVersion.isAfterOrEq(MinecraftVersion.parse("1.20.5")) && model_data.hasTooltipStyle()) meta.setTooltipStyle(model_data.getTooltipStyle());
			if (ServerVersion.isAfterOrEq(MinecraftVersion.parse("1.21.3")) && model_data.hasItemModel()) meta.setItemModel(model_data.getItemModel());
			if (ServerVersion.isAfterOrEq(MinecraftVersion.parse("1.21.4")) && model_data.hasModelDataComponent()) meta.setCustomModelDataComponent(model_data.getModelDataComponent());
