...
public class EditMaxStackButton extends ItemEditorButton {

	public Icon<ItemEditorMenuContext> getIcon() {
		return IconBuilder.<ItemEditorMenuContext>create().material(Material.BUCKET).name("Set Max Stack").lore(ctx -> List.of(
				"<gray>Current: <white>" + ctx.item().getMaxStack(),
				"",
				CasterColorCode.ORANGE + "✎ <gray>L - Change Max Stack",
			...
			ChatPrompt.listen(p, i -> {
				...
				ctx.item().setMaxStack(value.get());
				return Response.ACCEPTED;
			}).thenAcceptSync(in -> ctx.open(p));
		}).rightClick((ctx, p) -> {
			ctx.item().removeMaxStack();
		}).build();
	}
}
