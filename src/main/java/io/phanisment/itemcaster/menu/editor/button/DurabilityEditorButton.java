...
public class DurabilityEditorButton implements IEditorButton {

	@Override
	public ItemBuilder icon(CasterItem item) {
		return new ItemBuilder(Material.OAK_SIGN)
			.name(Legacy.serializer("<white>Edit Durability"))
			.lore(Legacy.serializer(
				"",
				"<gray>Left - Click to change durability",
	...
	public void left(InventoryClickEvent e, CasterItem item) {
		...
		CasterLogger.send(player, "<color:yellow>Enter the new durability for the item. Type 'cancel' to cancel.");
		ChatPrompt.listen(player, i -> {
			...
			try {
				item.setDurability(Integer.parseInt(i));
