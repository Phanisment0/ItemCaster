...
public class CasterItem {
	...
	public CasterItem(MythicItem mi) {
		...
		String model = config.getString("ModelItem");
		if (model != null) this.model_data = new ModelData(Identifier.of(model), mi);
	...
	public void applyData(MythicMobItemGenerateEvent e) {
		...
		if (Storage.hasResourcePack() && (model_data != null && model_data.hasItemModel())) model_data.applyModel(item);
	...
	public void setModelData(Identifier model) {
		if (model != null) model_data = new ModelData(model, mi);
		else model_data = null;
		save("ModelItem", model.toString());
	}
