...
import io.phanisment.itemcaster.skill.AbilitiesContainer;
...
public class CasterItem {
	...
	private AbilitiesContainer abilities;
	...
	public CasterItem(MythicItem mi) {
		...
		try {
			...
			List<Map<?, ?>> ability_list = config.getMapList("Abilities");
			for (Map<?, ?> value : ability_list) list.add(new SkillAttribute((Map<String, Object>)value));
			if (list != null) this.abilities = new AbilitiesContainer(list);
		} catch (Exception e) {
